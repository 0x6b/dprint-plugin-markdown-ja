...
use dprint_core::{configuration::resolve_new_line_kind, formatting::*};

use super::{
    configuration::Configuration,
    generation::{
        file_has_ignore_file_directive, generate, parse_cmark_ast, strip_metadata_header, Context,
    },
};
...
pub fn trace_file(
    ...
    let (source_file, markdown_text) = match parse_source_file(file_text, config).unwrap() {
        ParseFileResult::IgnoreFile => {
            panic!("Cannot trace file because it has an ignore file comment.")
        }
        ParseFileResult::SourceFile(file) => file,
    };
...
fn parse_source_file<'a>(
    file_text: &'a str,
    config: &Configuration,
) -> Result<ParseFileResult<'a>> {
    // check for the presence of an dprint-ignore-file comment before parsing
    if file_has_ignore_file_directive(
        strip_metadata_header(file_text),
