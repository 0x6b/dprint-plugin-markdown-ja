...
use super::parsing::{
    parse_image as parse_image_from_text, parse_link_reference, parse_link_reference_definitions,
};
use crate::generation::{common::*, trim_spaces_and_newlines, trim_start_spaces_and_newlines};
...
impl<'a> EventIterator<'a> {
    ...
    pub fn get_range_for_start(&self, start: usize) -> Range {
        Range { start, end: self.last_range.end }
    }
    ...
    pub fn get_not_implemented(&self) -> Node {
        NotImplemented { range: self.last_range.clone() }.into()
    }
}
...
pub fn parse_cmark_ast(markdown_text: &str) -> Result<SourceFile, ParseError> {
    ...
    Ok(SourceFile { children, range: iterator.get_range_for_start(0) })
}
...
fn parse_references(
    ...
    if let Some(last_event_end) = last_event_end {
        if last_event_end < end {
            let references = parse_link_reference_definitions(
                last_event_end,
                &iterator.file_text[last_event_end..end],
            )?;
...
fn parse_event(event: Event, iterator: &mut EventIterator) -> Result<Node, ParseError> {
    match event {
        ...
        Event::FootnoteReference(reference) => {
            parse_footnote_reference(reference, iterator).map(|x| x.into())
        }
        ...
        Event::TaskListMarker(is_checked) => {
            Ok(TaskListMarker { range: iterator.get_last_range(), is_checked }.into())
        }
...
fn parse_start(start_tag: Tag, iterator: &mut EventIterator) -> Result<Node, ParseError> {
    match start_tag {
        ...
        Tag::FootnoteDefinition(label) => {
            parse_footnote_definition(label, iterator).map(|x| x.into())
        }
        ...
        Tag::Emphasis => {
            parse_text_decoration(TextDecorationKind::Emphasis, iterator).map(|x| x.into())
        }
        Tag::Strong => {
            parse_text_decoration(TextDecorationKind::Strong, iterator).map(|x| x.into())
        }
        Tag::Strikethrough => {
            parse_text_decoration(TextDecorationKind::Strikethrough, iterator).map(|x| x.into())
        }
        Tag::Link {
            link_type,
        ...
        Tag::MetadataBlock(metadata_block_kind) => {
            parse_metadata(metadata_block_kind, iterator).map(|x| x.into())
        }
    ...
    while let Some(event) = iterator.next() {
        match event {
            ...
            Event::TaskListMarker(is_checked) if marker.is_none() => {
                marker = Some(TaskListMarker { range: iterator.get_last_range(), is_checked });
            }
            _ => children.push(parse_event(event, iterator)?),
        }
    }
...
fn parse_text_decoration(
    ...
    iterator: &mut EventIterator,
) -> Result<TextDecoration, ParseError> {
    let start = iterator.start();
    let mut children = Vec::new();
...
fn parse_html(text: CowStr, iterator: &mut EventIterator) -> Result<Html, ParseError> {
    ...
    Ok(Html { range: Range { start, end: start + text.len() } })
}

fn parse_display_math(
    _text: CowStr,
    iterator: &mut EventIterator,
) -> Result<DisplayMath, ParseError> {
    Ok(DisplayMath { range: iterator.get_last_range() })
}

fn parse_inline_math(
    _text: CowStr,
    iterator: &mut EventIterator,
) -> Result<InlineMath, ParseError> {
    Ok(InlineMath { range: iterator.get_last_range() })
}
...
fn parse_footnote_reference(
    name: CowStr,
    iterator: &mut EventIterator,
) -> Result<FootnoteReference, ParseError> {
...
fn parse_footnote_definition(
    name: CowStr,
    iterator: &mut EventIterator,
) -> Result<FootnoteDefinition, ParseError> {
    let start = iterator.start();
    let mut children = Vec::new();
...
fn parse_link(
    ...
    match link_type {
        ...
        LinkType::Shortcut | LinkType::ShortcutUnknown => {
            Ok(ShortcutLink { range, children }.into())
        }
...
fn parse_table_head(iterator: &mut EventIterator) -> Result<TableHead, ParseError> {
    ...
    Ok(TableHead { range: iterator.get_range_for_start(start), cells })
}
...
fn parse_table_row(iterator: &mut EventIterator) -> Result<TableRow, ParseError> {
    ...
    Ok(TableRow { range: iterator.get_range_for_start(start), cells })
}
...
fn parse_item(iterator: &mut EventIterator) -> Result<Item, ParseError> {
    ...
    Ok(Item { range, marker, children, sub_lists })
}
...
fn parse_metadata(
    ...
    while let Some(event) = iterator.next() {
        match event {
            ...
            // assuming here that pulldown-cmark doesn't tokenize anything beyond text and line
            // breaks between metadata block markers then everything else should be a
            // parse error here
            _ => {
                return Err(ParseError::new(
                    iterator.get_last_range(),
