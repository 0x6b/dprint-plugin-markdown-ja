...
pub fn parse_image(offset: usize, text: &str, link_type: LinkType) -> Result<Node, ParseError> {
    ...
    match link_type {
        ...
        | LinkType::Collapsed
        | LinkType::CollapsedUnknown => parse_reference(start_pos, &mut char_scanner),
        LinkType::Shortcut | LinkType::ShortcutUnknown | LinkType::Email | LinkType::Autolink => {
            Err(ParseError::new(
...
fn parse_inline(start_pos: usize, char_scanner: &mut CharScanner) -> Result<Node, ParseError> {
    ...
    Ok(InlineImage {
        range: Range { start: start_pos, end: char_scanner.end() },
        text,
        url,
        title,
    }
