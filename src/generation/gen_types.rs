...
use anyhow::Result;
use dprint_core::formatting::{PrintItemPath, PrintItems, Signal};
use regex::Regex;
...
impl<'a> Context<'a> {
            ...
            match kind {
                ...
                MemoizedRcPathKind::StartWithSingleIndent(times) => {
                    items.push_optional_path(
                        self.get_memoized_rc_path(MemoizedRcPathKind::StartIndent(times)),
                    );
                    for _ in 0..times {
    ...
    pub fn format_text<'b>(&mut self, tag: &str, text: &'b str) -> FormatResult {
        let line_width =
            std::cmp::max(10, self.configuration.line_width as i32 - self.indent_level as i32)
                as u32;
