...
use super::{common::*, gen_types::*, utils};
            ...
            if matches!(
                ...
                match last_node {
                    ...
                    | Node::InlineMath(_) => {
                        ...
                        let new_line_count =
                            context.get_new_lines_in_range(between_range.0, between_range.1);
                        ...
                        if new_line_count == 1 {
                            ...
                            // > Some note.
                            let is_callout =
                                if context.is_in_block_quote() && matches!(node, Node::Text(_)) {
                            ...
                            } else if matches!(last_node, Node::Text(_))
                            ...
                            if needs_space {
                                if node.starts_with_list_word() {
                                    ...
                                    if matches!(last_node, Node::Text(_))
                                        && matches!(node, Node::Text(_))
                                    {
                                    }
        ...
        if let Node::Html(html) = node {
            ...
            if context.ignore_regex.is_match(html_text) {
                ...
                if let Some(node) = node_iterator.next() {
                    ...
                    let text_start =
                        utils::get_leading_non_space_tab_byte_pos(context.file_text, range.start);
            ...
            match print_item {
                PrintItem::String(text) => {
                    items.push_condition(if_true(
                        ...
                        {
                            let mut items = PrintItems::new();
                            items.push_optional_path(context.get_memoized_rc_path(
                                MemoizedRcPathKind::FinishIndent(indent_level),
                            ));
                            items.push_string(">".repeat(block_quote_count));
                            items.push_space();
                            items.push_optional_path(context.get_memoized_rc_path(
                                MemoizedRcPathKind::StartWithSingleIndent(indent_level),
                            ));
                            items
                        },
                    ));
                ...
                PrintItem::Signal(Signal::NewLine) => {
                    items.push_condition(if_true(
                        ...
                        condition_resolvers::is_start_of_line(),
                        {
                            ...
                            items.push_string(">".repeat(block_quote_count));
                            items.push_optional_path(context.get_memoized_rc_path(
                                MemoizedRcPathKind::StartIndent(indent_level),
                            ));
                            items
                        },
                    ));
...
fn is_callout_text(text: &str) -> bool {
    ...
    text.starts_with("[!")
        && text.ends_with("]")
        && text[2..text.len() - 1].chars().all(|c| c.is_ascii_uppercase())
}
...
fn gen_str(text: &str, context: &mut Context) -> PrintItems {
    ...
    impl<'a> TextBuilder<'a> {
        ...
        pub fn add_char(&mut self, character: char) {
            ...
            if character == '\n' || character == ' ' {
                if self.context.configuration.text_wrap == TextWrap::Maintain && character == '\n' {
                    ...
                    self.space_or_newline_if_needed(character);
                }
                return;
            }
        ...
        fn space_or_newline_if_needed(&mut self, character: char) {
            if let Some(current_word) = self.current_word.as_ref() {
                if should_add_space_between_chars(
                    current_word.as_str().chars().last().unwrap_or_default(),
                    character,
                ) {
...
fn gen_inline_link(link: &InlineLink, context: &mut Context) -> PrintItems {
    context.with_no_text_wrap(|context| {
        ...
        let single_line_text =
            get_items_text(ir_helpers::with_no_new_lines(generated_children_clone));
    ...
    context.mark_in_list(|context| {
        ...
        for (index, child) in list.children.iter().enumerate() {
            ...
            let prefix_text = if let Some(start_index) = list.start_index {
                ...
                let display_index =
                    if is_all_ones_list(list, context) { 1 } else { start_index + index as u64 };
            ...
            items.push_condition(if_true(
                "spaceIfHasChild",
                Rc::new(move |context| {
                    Some(!condition_helpers::is_at_same_position(context, after_child)?)
                }),
...
fn gen_item(item: &Item, context: &mut Context) -> PrintItems {
    ...
    items.extend(gen_task_list_marker_children(&item.children, item.marker.as_ref(), context));

    if !item.sub_lists.is_empty() {
        items.push_signal(Signal::NewLine);
        if utils::has_leading_blankline(
            item.sub_lists.first().unwrap().range().start,
            context.file_text,
        ) {
            items.push_signal(Signal::NewLine);
        }
...
fn gen_task_list_marker_children(
    ...
    if indent_child_index_end > 0 && indent_child_index_end != children.len() {
        items.push_signal(Signal::NewLine);
        if utils::has_leading_blankline(
            children[indent_child_index_end].range().start,
            context.file_text,
        ) {
            items.push_signal(Signal::NewLine);
        }
...
fn gen_table(table: &Table, context: &mut Context) -> PrintItems {
    ...
    fn get_divider_row(
        ...
        for (i, column_width) in column_widths.iter().enumerate() {
            let column_alignment =
                column_alignments.get(i).copied().unwrap_or(ColumnAlignment::None);
            if i == 0 {
                items.push_sc(sc!("| "));
            } else {
                items.push_space();
        ...
        for (i, (cell_items, cell_width)) in row_cells.into_iter().enumerate() {
            ...
            if difference > 0 {
                match column_alignment {
                    ...
                    ColumnAlignment::Center => {
                        if difference > 1 {
                            items.push_string(
                                " ".repeat((difference as f32 / 2_f32).floor() as usize),
                            )
                        }
                    }
            ...
            if difference > 0 {
                match column_alignment {
                    ...
                    ColumnAlignment::Center => {
                        items.push_string(" ".repeat((difference as f32 / 2_f32).ceil() as usize))
                    }
                    ColumnAlignment::Right => {}
                }
            }
    ...
    fn get_column_alignment_properties(
        column_alignment: ColumnAlignment,
    ) -> ColumnAlignmentProperties {
        let has_left_colon = column_alignment == ColumnAlignment::Left
            || column_alignment == ColumnAlignment::Center;
        let has_right_colon = column_alignment == ColumnAlignment::Right
            || column_alignment == ColumnAlignment::Center;
        ColumnAlignmentProperties { has_left_colon, has_right_colon }
    }
...
fn should_add_space_between_chars(prev: char, curr: char) -> bool {
  ...
  match (prev, curr, is_japanese(prev), is_japanese(curr)) {
    ...
    (_   , _   , true , false) | (_  , _, false, true )                                         => true,
    (_   , ' ' , false, _    ) | (' ', _, _    , false)                                         => true,
    _                                                                                           => false,
  }
}
