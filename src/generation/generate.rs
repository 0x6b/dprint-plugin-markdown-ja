...
fn should_add_space_between_chars(prev: char, curr: char) -> bool {
  ...
  let next_is_japanese = is_japanese(curr);

  if is_allowed(prev, curr) {
    return false;
  }
...
#[inline]
fn is_allowed(prev: char, curr: char) -> bool {
  match (is_japanese(prev), is_japanese(curr), prev, curr) {
    (true, true, _, _) => true,
    (true, _, _, '(') => true,
    (true, _, _, ')') => true,
    (_, true, '(', _) => true,
    ...
    (true, _, _, '~') => true,
    (_, true, '~', _) => true,
    (_, false, _, _) if is_japanese_symbols_and_punctuation(prev) => true,
    (false, _, _, _) if is_japanese_symbols_and_punctuation(curr) => true,
    _ => false,
  }
}
